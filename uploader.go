...
// PutClient represents the subset of the S3 interface we use in the Uploader,
// which facilitates test mocks.
type PutClient interface {
	PutObject(*s3.PutObjectInput) (*s3.PutObjectOutput, error)
}

// UploadOptions contains the configuration information for the Upload process,
// and exposes no public methods.
type UploadOptions struct {
	// Delimiter separates blobs within the S3 objects. If nil, defaults to '\n';
	// pass an empty byte array to disable.
	Delimiter []byte
	// Input provides a stream of blobs (byte arrays) to batch and upload to S3.
	Input <-chan []byte

	// Client is a pre-configured S3 client capable of writing objects to the
	// defined S3 Bucket.
	Client PutClient
	// Bucket is the S3 bucket into which batched uploads get saved.
	Bucket string

	// ConcurrentUploads is the number of concurrent S3 object writes we should
	// have going at any one point in time.
	ConcurrentUploads uint
	// BatchMaxBlobs is the maximum number of blobs to batch into a single object.
	// This should probably be pretty high, as BatchSizeBytes is intended to be
	// the primary batching driver.
	BatchMaxBlobs uint
	// BatchSizeBytes is the batch cutoff size, in bytes. All batches that do not
	// time out or run out of buffer space (in slices) will attempt to just exceed
	// this.
	BatchSizeBytes uint64
	// BatchWindow is the batch cutoff duration; when a batch has been collecting
	// for at least this long, it'll be considered complete and uploaded. This
	// should be defined by the amount of data you're willing to lose.
	BatchWindow time.Duration

	// GetKey generates the key string given the time at which the first blob
	// arrived. It is invoked immediately when a buffer is closed, and used to
	// name/locate the object as it gets uploaded.
	GetKey func(time.Time) string

	// errors is the output channel for emitting errors. These are generally non-
	// fatal, except for any errors that occur in the Upload function itself. This
	// channel closes when the uploader stops - usually when the Input channel was
	// closed and all blobs received have been written to S3.
	errors chan<- error

	// bufferPool is a pool of buffers sized according to ConcurrentUploads. One
	// buffer corresponds to one potential goroutine. These buffers are sized
	// according to BatchMaxBlobs, and store the blobs provided.
...
// uploadObject runs in a new goroutine and handles uploading a batch of blobs
// to S3, including compressing the data and inserting delimiters.
...
// contextWithTimeout is a helper function that only derives a new context with
// a timeout if the BatchWindow is configured.
...
// getBatch collects a batch together and returns it and additional metadata.
// Respects the various batch parameters, including BatchSizeBytes, BatchWindow,
// and BatchMaxBlobs.
...
// run aggregates batches and spins up goroutines to upload the batches as S3
// objects. When the Input is closed and all uploads are complete, this will
// drain the buffer pool, close the errors channel, and exit.
...
// Upload consumes a channel containing byte arrays (blobs), joins them with a
// delimiter in batches, and uploads the batches to S3.
